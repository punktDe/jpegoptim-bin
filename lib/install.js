...
bin.run(['--version'], err => {
	if (err) {
        ...
        const builder = new BinBuild()
			.src(`https://raw.githubusercontent.com/downloads/punktDe/jpegoptim-bin/vendor/freebsd/jpegoptim`)
        	.cmd(`./configure --prefix="${bin.dest()}" --bindir="${bin.dest()}" --with-libjpeg=/usr/local`)
            .cmd('make install')
